...
public class Database {
	...
	public void insertAnnonce(Annonce annonce) throws SQLException {
		String query = "INSERT INTO annonce (id, title, description, startDate, endDate, postsNum, secteur, fonction, experience, etudeLevel, contratDetails, url, siteName, adresseEntreprise, siteWebEntreprise, nomEntreprise, descriptionEntreprise, region, city, industry, traitsPersonnalite, competencesRequises, softSkills, competencesRecommandees, langue, niveauLangue, salaire, avantagesSociaux, teletravail) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

		try (PreparedStatement stmt = connection.prepareStatement(query)) {
			stmt.setInt(1, annonce.getId());
			stmt.setString(2, annonce.getTitle());
			stmt.setString(3, annonce.getDescription());
			stmt.setString(4, annonce.getStartDate());
			stmt.setString(5, annonce.getEndDate());
			stmt.setInt(6, annonce.getPostsNum());
			stmt.setString(7, annonce.getSecteur());
			stmt.setString(8, annonce.getFonction());
			stmt.setString(9, annonce.getExperience());
			stmt.setString(10, annonce.getEtudeLevel());
			stmt.setString(11, annonce.getContratDetails());
			stmt.setString(12, annonce.getUrl());
			stmt.setString(13, annonce.getSiteName());
			stmt.setString(14, annonce.getAdresseEntreprise());
			stmt.setString(15, annonce.getSiteWebEntreprise());
			stmt.setString(16, annonce.getNomEntreprise());
			stmt.setString(17, annonce.getDescriptionEntreprise());
			stmt.setString(18, annonce.getRegion());
			stmt.setString(19, annonce.getCity());
			stmt.setString(20, annonce.getIndustry());
			stmt.setString(21, annonce.getTraitsPersonnalite());
			stmt.setString(22, annonce.getCompetencesRequises());
			stmt.setString(23, annonce.getSoftSkills());
			stmt.setString(24, annonce.getCompetencesRecommandees());
			stmt.setString(25, annonce.getLangue());
			stmt.setString(26, annonce.getNiveauLangue());
			stmt.setString(27, annonce.getSalaire());
			stmt.setString(28, annonce.getAvantagesSociaux());
			stmt.setString(29, annonce.getTeletravail());
			stmt.executeUpdate();
		}
	}
	...
	public void updateAnnonce(Annonce annonce) throws SQLException {
		String query = "UPDATE annonce SET title = ?, description = ?, startDate = ?, endDate = ?, postsNum = ?, secteur = ?, fonction = ?, experience = ?, etudeLevel = ?, contratDetails = ?, url = ?, siteName = ?, adresseEntreprise = ?, siteWebEntreprise = ?, nomEntreprise = ?, descriptionEntreprise = ?, region = ?, city = ?, industry = ?, traitsPersonnalite = ?, competencesRequises = ?, softSkills = ?, competencesRecommandees = ?, langue = ?, niveauLangue = ?, salaire = ?, avantagesSociaux = ?, teletravail = ? WHERE id = ?";
